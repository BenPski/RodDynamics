...
class Load(metaclass=ABCMeta):
    ...
    @abstractmethod
    def dist_load(self, g, xi, eta, xi_dot, eta_dot, rod, q, s):
...
class Gravity(Load):
    ...
    def dist_load(self, g, xi, eta, xi_dot, eta_dot, rod, q, s):
        R = g[:3, :3]
        A_bar = 0
        coeff = rod.body.grav_data(s)
        B_bar = coeff * np.concatenate([np.array([0, 0, 0]), R.T @ self.grav])
...
class PointLoadBody(Load):
    ...
    def dist_load(self, g, xi, eta, xi_dot, eta_dot, rod, q, s):
        return super().dist_load(g, xi, eta, xi_dot, eta_dot, rod, q, s)
