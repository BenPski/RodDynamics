...
from loads import Gravity, Cables, TipLoad
...
if __name__ == "__main__":
    ...
    # the rod itself
    # r1 = Rod(1e-2, 10e-2, 1e6, 1e3, 0, N, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    # # ax=r1.plot(ax)
    # # plt.pause(0.01)
    # tip1.append(r1.g[-1, 9])
    ...
    # # the rod in a series
    # sys = Series([r1])
    # tip2.append(sys.rods[-1].g[-1,9])
    # for i in range(steps):
    #     print(i, '/', steps)
    #     sys.step(dt, np.array([-0.4]))
    #     tip2.append(sys.rods[-1].g[-1,9])
    # ax.plot(tip2)

    # the rod split into 2 segments
    r1 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    r2 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    r3 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    E = []
    sys = Series([r1, r2, r3])

    # ax=sys.plot(ax)
    # plt.pause(0.01)
    tip3.append(sys.g[-1, 9])
    for i in range(steps):
        print(i, '/', steps)
        sys.step(dt, np.array([0]))
        # rod.step(dt, np.array([0]))
        # E.append(sys.energy())
        # ax = sys.plot(ax)
        # plt.pause(0.01)
        tip3.append(sys.g[-1, 9])
    ...
    r1 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads+[TipLoad(np.array([0,1,0,0,0,0]))])
    r2 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    r3 = Rod(1e-2, 10e-2 / 3, 1e6, 1e3, 0, N // 3, xi_init=lambda s: np.array([0, 0, 0, 0, 0, 1]), loads=loads)
    E = []
    sys = Series([r1, r2, r3])
